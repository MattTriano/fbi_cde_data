...
select
    ...
    {{ parse_nibrs_date('ori_added') }}                          as ori_added,
    {{ parse_nibrs_date('ori_nibrs', '1900') }}                  as ori_started_nibrs,
    nullif(upper(trim(city)), '')                                as city,
    trim(state_abbr)                                             as state_abbr,
    trim(pop_group)                                              as pop_group,
    trim(division)                                               as division,
    {{ trim_and_stdize_nulls('region', '[*\s\n]') }}             as region,
    {{ trim_and_stdize_nulls('agency_ind') }}                    as agency_ind,
    trim(core_city)::boolean                                     as core_city,
    {{ trim_and_stdize_nulls('covered_by_ori') }}                as covered_by_ori,
    trim(fbi_field_office)                                       as fbi_field_office,
    {{ trim_and_stdize_nulls('judicial_district') }}             as judicial_district,
    {{ trim_and_stdize_nulls('nibrs_flag') }}                    as nibrs_flag,
    {{ parse_nibrs_date('inactive_date') }}                      as inactive_date,
    ...
    regexp_replace(trim(ind_1_6_12), '', '0')::smallint          as ind_1_6_12,
    ...
    {{ trim_and_stdize_nulls('cnty1_fips') }}                    as county1_fips,
    {{ trim_and_stdize_nulls('cnty2_fips') }}                    as county2_fips,
    {{ trim_and_stdize_nulls('cnty3_fips') }}                    as county3_fips,
    {{ trim_and_stdize_nulls('cnty4_fips') }}                    as county4_fips,
    {{ trim_and_stdize_nulls('cnty5_fips') }}                    as county5_fips
from unified_parts
